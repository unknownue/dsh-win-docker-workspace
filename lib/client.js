...
//#endregion
//#region node_modules/.pnpm/zustand@4.4.7_@types+react@19.2.18_immer@10.2.0_react@18.3.1/node_modules/zustand/esm/vanilla.mjs
...
//#endregion
//#region node_modules/.pnpm/zustand@4.4.7_@types+react@19.2.18_immer@10.2.0_react@18.3.1/node_modules/zustand/esm/middleware.mjs
...
//#endregion
//#region node_modules/.pnpm/@deepseek-ai+dsh-client-sto_f8f997157dbebd3ebb0673beb8e1e886/node_modules/@deepseek-ai/dsh-client-store/lib/index.js
/**
...
function apply(ctx) {
	...
	ctx.effect(() => {
		...
		const refreshRoster = () => {
			remote.agentPresets.list().then((result) => {
				if (!result.ok) return;
				const presets = result.value?.presets ?? [];
				variants = new Set(presets.filter((entry) => entry.broken === void 0 && entry.id.startsWith("win-docker-")).map((entry) => entry.id));
		...
		const maybeBind = () => {
			...
			for (const id of state.ids) {
				...
				const current = summary.projectionValues?.agentPreset ?? void 0;
				...
				inFlight.add(id);
				remote.agentPresets.select(id, target).then((response) => {
					if (response.ok) {}
				}).catch(() => {
