...
import { a as listWorkspaces, d as isValidShellName, f as isWindowsDrivePath, h as normalizeWindowsPath, l as containsMount, n as containerPathOf, o as resolveAnchorPath, p as mapContainerToHost, r as getWorkspace, s as setWorkspace, u as isValidContainerName } from "./win-docker-workspaces-BeycJyLC.js";
...
*/
const PERSONA_SCALAR_PATTERNS = [/^(\s*)prefix: [>|-]/, /^(\s*)text: [>|-]/];
/**
...
*/
function personaScalarIndex(block) {
...
async function dispatch(method, params) {
	switch (method) {
		...
		case "setWorkspace": {
			...
			return { anchor: setWorkspace(path, container, shell) };
		}
...
function apply(ctx, config) {
	...
	if (shellEnv !== void 0) ctx.effect(() => shellEnv.register({
		...
		variables: {
			...
			DSH_DOCKER_WORKSPACE: { description: "The in-container workspace root of the calling session (e.g. C:\\workspace), when the session cwd is a Docker workspace path." },
		...
		resolve(execution) {
			...
			if (anchor !== void 0) {
				...
				return {
					...
					...entry.shell === void 0 || entry.shell === "" ? {} : { DSH_DOCKER_SHELL: entry.shell }
				};
			}
